...
import { 
  Heart, LogOut, User, Settings, ChevronDown, ShieldCheck, Plus, CheckCircle2 
} from 'lucide-react';
...
export function CaregiverHeader({ onNavigate, showSuccessBanner }: CaregiverHeaderProps) {
  ...
  return (
    <>
      ...
      {showSuccessBanner && (
        <div className="fixed top-16 left-0 right-0 z-50 bg-emerald-600 text-white py-2 px-4 text-center text-xs font-bold shadow-md flex items-center justify-center gap-2 animate-bounce">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-xl border-b border-gray-100 shadow-xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              type="button"
              onClick={() => onNavigate && onNavigate('create-offer')}
              className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-2xl bg-teal-900 text-white text-xs font-bold shadow-md shadow-teal-900/20 hover:bg-teal-800 transition-all cursor-pointer"
            >
                  ...
                  <div className="absolute right-0 mt-2 w-64 z-[99999] bg-white border border-gray-200 rounded-2xl shadow-2xl py-2 pointer-events-auto">
                    ...
                    <div className="py-1">
                      <button
                        ...
                        className="w-full flex items-center gap-2.5 px-4 py-2.5 text-xs font-bold text-gray-700 hover:bg-teal-50 hover:text-teal-900 transition-colors cursor-pointer text-left"
                      >
                        <User className="w-4 h-4 text-teal-600" /> Profil & Angaben bearbeiten
                      </button>
                      <button
                        type="button"
                        onClick={() => { setDropdownOpen(false); onNavigate && onNavigate('availability'); }}
                        className="w-full flex items-center gap-2.5 px-4 py-2.5 text-xs font-bold text-gray-700 hover:bg-teal-50 hover:text-teal-900 transition-colors cursor-pointer text-left"
                      >
